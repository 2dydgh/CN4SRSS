...
class RecordBox():
    ...
    def __init__(self, name='name', is_print=True, print_interval=1, will_update_graph=True, **kargs):
        ...
        self.graph_update_count = None              # graph 갱신 요청 횟수
        
        if self.will_update_graph:
            try:
                self.graph_update_interval = int(kargs['graph_update_interval'])
            except:
                self.graph_update_interval = 100    # (int) graph 갱신 주기
            
            if "test" in self.name or "Test" in self.name:
                self.graph_update_interval = 1
            
            print(self.print_head, "This will update graph every", self.graph_update_interval, "epoch.")
        else:
            self.graph_update_interval  = None
            print(self.print_head, "This will NOT update graph!")
    ...
    def update_epoch(self, is_return = False
        ...
        if self.graph_update_interval is None and _update_graph:
            self.graph_update_interval = 1
        ...
        if _update_graph:
            if self.graph_update_count is None:
                self.graph_update_count = 1
                print(self.print_head, "graph initialized.")
                ...
                if self.graph_update_count >= self.graph_update_interval:
                    ...
                    self.update_graph(is_show = self.is_show, is_save = self.is_save, path = self.path)
                    self.graph_update_count = 0  # count reset
                else:
                    print(self.print_head, "graph update requested:"
                         ,self.graph_update_count, "/", self.graph_update_interval
                         )
    ...
    def update_graph(self, is_show = False, is_save = True, path = "./"):
        ...
        plt.plot(list_x_labels, self.record_epoch_item)                                         # main graph
        plt.scatter([int(self.total_max[0])], [self.total_max[-1]], c = 'red', s = 100)         # MAX point
        plt.scatter([int(self.total_min[0])], [self.total_min[-1]], c = 'lawngreen', s = 100)   # min point
        plt.xlabel("\nEpoch")
...
class RecordBox4IoUs():
    ...
    def __init__(self, name='name', labels = 11, is_print = True, print_interval = 1, will_update_graph=True, **kargs):
        ...
        if self.will_update_graph:
            try:
                ...
                self.graph_update_interval = 100     # graph 갱신 주기
    ...
    def update_graph(self, is_show = False, is_save = True, path = "./"):
        ...
        plt.plot(list_x_labels, list_y_miou)                                                    # main graph
        plt.scatter([int(self.total_max[0])], [self.total_max[-1]], c = 'red', s = 100)         # MAX point
        plt.scatter([int(self.total_min[0])], [self.total_min[-1]], c = 'lawngreen', s = 100)   # min point
...
class Custom_Dataset_V6(data.Dataset):
    ...
    def __init__(self, **kargs):
        ...
        try:
            self.load_everytime = kargs['load_everytime']       # 이미지를 RAM에 올려두지 않고, 매번 새로 불러올 경우에 True
        except:
            self.load_everytime = False                         # default는 이미지를 한번만 불러오도록 RAM에 올려두고 사용하는 방식
            # 아래 항목은 외부파일로 default 옵션을 True로 바꾸기 위한 코드
            list_load_everytime = ["./_load_everytime", "/root/LAB/_load_everytime"]
            for i_path in list_load_everytime:
                try:
                    if os.path.isdir(i_path):
                        print(self.name_func, "(", i_path, ") found. load_everytime set True.")
                        self.load_everytime = True
                except:
                    pass
            if not self.load_everytime:
                _str = ", ".join(list_load_everytime)
                print("Not found: " + _str)
                print("load_everytime set False.")
            
        
        if not self.load_everytime:
            print(self.name_func, "load_everytime:", self.load_everytime, "\nData will be loaded only once from drive.")
            print("This will require high RAM memory.")
        else:
            print(self.name_func, "load_everytime:", self.load_everytime, "\nData will be loaded everytime from drive.")
            print("This will require low RAM memory.")
        
        self.mp_method = mp.get_start_method()
        print(self.name_func, "Detected multi-processing start method:", self.mp_method)  # spawn, fork
        ...
        self.list_file_img = sorted(os.listdir(self.in_path_dataset + self.in_category + self.in_name_folder_image))
        ...
        if self.is_return_label:
            ...
            self.list_file_lab = sorted(os.listdir(self.in_path_dataset + self.in_category + self.in_name_folder_label)) # (list with str) file names
        ...
        if self.is_return_image_lr:
            ...
            self.list_file_img_lr = sorted(os.listdir(self.in_path_dlc + self.in_name_folder_image))
        
        
        #--- make dict with pil
        
        self.dict_pil_raw_img_hr = {}                                               # (dict with pil) raw HR image or (dict with str) raw HR image path
        
        if self.is_return_label:
            self.dict_pil_raw_lab_hr = {}                                           # (dict with pil) raw HR label or (dict with str) raw HR label path
        
        if self.is_return_image_lr:
            self.dict_pil_raw_img_lr = {}                                           # (dict with pil) raw LR image or (dict with str) raw LR image path
        ...
        for i_file in range(len(self.list_file_img)):
            ...
            if not self.load_everytime:
                ...
                if self._pack is None:
                    ...
                    # self.dict_pil_raw_img_hr[self.list_file_img[i_file]] = path_img_hr
                    self.dict_pil_raw_img_hr[self.list_file_img[i_file]] = self._pack(pil_img_hr_raw)
            else:
                self.dict_pil_raw_img_hr[self.list_file_img[i_file]] = path_img_hr
            ...
            if self.is_return_label:
                ...
                if not self.load_everytime:
                    ...
                    if self._pack is None:
                        ...
                        # self.dict_pil_raw_lab_hr[self.list_file_lab[i_file]] = path_lab_hr
                        self.dict_pil_raw_lab_hr[self.list_file_lab[i_file]] = self._pack(pil_lab_hr_raw)
                else:
                    self.dict_pil_raw_lab_hr[self.list_file_lab[i_file]] = path_lab_hr
            ...
            for i_file in range(len(self.list_file_img_lr)):
                ...
                if not self.load_everytime:
                    ...
                    if self._pack is None:
                        ...
                        # self.dict_pil_raw_img_lr[self.list_file_img_lr[i_file]] = path_img_lr
                        self.dict_pil_raw_img_lr[self.list_file_img_lr[i_file]] = self._pack(pil_img_lr_raw)
                else:
                    self.dict_pil_raw_img_lr[self.list_file_img_lr[i_file]] = path_img_lr
    ...
    def __getitem__(self, idx):
        ...
        if not self.load_everytime:
            if self._unpack is None:
                ...
                # pil_img_hr_raw = Image.open(self.dict_pil_raw_img_hr[self.list_file_img[idx]])
                pil_img_hr_raw = self._unpack(self.dict_pil_raw_img_hr[self.list_file_img[idx]])
        else:
            pil_img_hr_raw = Image.open(self.dict_pil_raw_img_hr[self.list_file_img[idx]])
        ...
        if self.is_return_label:
            if not self.load_everytime:
                if self._unpack is None:
                    ...
                    # pil_lab_hr_raw = Image.open(self.dict_pil_raw_lab_hr[self.list_file_lab[idx]])
                    pil_lab_hr_raw = self._unpack(self.dict_pil_raw_lab_hr[self.list_file_lab[idx]])
            else:
                pil_lab_hr_raw = Image.open(self.dict_pil_raw_lab_hr[self.list_file_lab[idx]])
        ...
        if self.is_return_image_lr:
            if not self.load_everytime:
                if self._unpack is None:
                    ...
                    # pil_img_lr_raw = Image.open(self.dict_pil_raw_img_lr[self.list_file_img[idx]])
                    pil_img_lr_raw = self._unpack(self.dict_pil_raw_img_lr[self.list_file_img[idx]])
            else:
                pil_img_lr_raw = Image.open(self.dict_pil_raw_img_lr[self.list_file_img[idx]])
        ...
        if self.is_force_fix:
            # Patch 생성 및 크기 부족시, 강제 margin 추가 -> 세부 옵션은 CamVid 11라벨 기준
