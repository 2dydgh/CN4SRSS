...
class PklLoader():
    def __init__(self, **kwargs):
        ...
        if self.use_pkl:
            ...
            # test pickle 교체
            path_test_pkl_key = "test_" + path_pkl_key
            self.LogText_.add_str(_HEAD, "check designated path for test pkl:", path_test_pkl_key)
            try:
                self.path_test_pkl = dict_path[path_test_pkl_key]
                if self.path_test_pkl == "None":
                    self.path_test_pkl = None
            except:
                self.path_test_pkl = None
            self.LogText_.add_str(_HEAD, "detected path for test pkl:", str(self.path_test_pkl))
