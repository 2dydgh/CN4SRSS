# paper 1st - model 07 trainer
#
# requirement
#   workspace           v 4.3.1
#   DLCs                v 2.0.1
#   Prop_9_NEW_Ab_07    v 2023-01-11
# 
# 2023_10_08 init
#   trainer_1_3_3_fix_2.py, trainer_p1.py 기반
#
...
import time
import warnings
_str = "\n\n---[ Proposed 1nd paper trainer version: " + str(_tv) + " ]---\n"
warnings.warn(_str)
...
#@@<
# GPU 타이머 사용 시에는 torch.cuda.synchronize()가 필요함
...
def one_epoch_cn4srss(**kargs):
    ...
    # SR(S) Inference Times 저장할 리스트
    inference_times_sr_s = []
    ...
    for dataloader_items in dataloader_input:
        ...
        try:
            ...
            # print("dl_ts_lab_hr_void max", torch.max(dl_ts_lab_hr_void))
        except:
            dl_ts_lab_hr    = None
        try:
        ...
        if i_mode == "train":
            with torch.cuda.amp.autocast(enabled=True):
                tensor_out_if, _, tensor_out_sr, tensor_out_seg = model_l(dl_ts_img_lr)
                
                # label 예측결과 softmax 시행
                tensor_out_seg_softmax = F.softmax(tensor_out_seg, dim = 1)
                
                if CALC_WITH_LOGIT:
                    # loss_l = criterion_l.calc_v6(tensor_out_seg, tensor_out_sr, dl_ts_lab_hr, dl_ts_img_hr
                                                # ,pred_img_1=tensor_out_if
                                                # ,is_AMP     = True
                                                # ,is_onehot  = True
                                                # )
                    ...
                    if HP_LABEL_ONEHOT_ENCODE:
                        # print("torch.clamp(tensor_out_seg_softmax - dl_ts_lab_hr_void.to(device), 0, 1) =", torch.min(torch.clamp(tensor_out_seg_softmax - dl_ts_lab_hr_void.to(device), 0, 1)))
                        loss_l = criterion_l.calc_v6(torch.clamp(tensor_out_seg_softmax - dl_ts_lab_hr_void.to(device), 0, 1), tensor_out_sr, dl_ts_lab_hr, dl_ts_img_hr
                                                    ,pred_img_1=tensor_out_if
                                                    ,is_AMP     = True
                                                    ,is_onehot  = True
                                                    )
            ...
            with torch.no_grad():
                start_time_sr_s = measure_time_start()  # SR (S) 시작 시간 측정
                tensor_out_if, _, tensor_out_sr, tensor_out_seg = model_l(dl_ts_img_lr)
                ...
                # softmax 값을 바탕으로 label image 형태로 tensor 생성 (형태 변경 4D [B, C, H, W] -> 3D [B, H, W]) -> 이미지 생성에 사용됨
                tensor_out_seg_label = torch.argmax(tensor_out_seg_softmax.clone().detach().requires_grad_(False), dim = 1)
                # SR (S) 모델 inference 시간 기록 끝
                inference_time_sr_s = measure_time_end(start_time_sr_s)  # SR (S) 끝 시간 측정
                inference_times_sr_s.append(inference_time_sr_s)  # 리스트에 추가
    ...
    if i_mode == "val":
        ...
        tmp_is_best = None
        if inference_times_sr_s:
            average_inference_time_sr_s = sum(inference_times_sr_s) / len(inference_times_sr_s)
            print(f"\n--- Average Inference Time for SR (S): {average_inference_time_sr_s:.6f} seconds ---\n")
    #>>>check_point, state_dict 저장
...
def trainer_(**kargs):
    ...
    for i_epoch in range(HP_EPOCH):
        ...
        for i_mode in list_mode:
            ...
            is_best = one_epoch_cn4srss(WILL_SAVE_IMAGE = WILL_SAVE_IMAGE
