...
def trainer_(**kargs):
    ...
    try:
        ...
        #_str = "HP_DATASET_NAME -> 사용할 데이터셋의 종류를 입력해 주세요. (정식지원목록: CamVid, MiniCity)"
        #sys.exit(_str)
    ...
    for i_epoch in range(HP_EPOCH):
        ...
        for i_mode in list_mode:
            ...
            if i_mode == "test" and i_epoch > -1:   # 이제 첫 n epoch 동안 무조건 test 시행하는거 제거함
                if TRAINER_MODE == "SSSR":
            ...
            for dataloader_items in dataloader_input:
                ...
                if i_mode == "train":
                    ...
                    try:
                        ...
                        flag_error += 1
                        update_dict_v2("", "in " + str(i_epoch + 1) + " " + str(i_batch + 1)
                                      ,"", "loss.backward() 실패"
                                      ,in_dict = dict_log_error
                                      ,in_print_head = "dict_log_error"
                                      ,is_print = False
            ...
            if i_mode == "val":
                if TRAINER_MODE == "SSSR":
                    tmp_is_best = rb_val_ious.is_best_max   # mIoU 기준
                elif TRAINER_MODE == "SR":
                    tmp_is_best = rb_val_psnr.is_best_max   # PSNR 기준
                elif TRAINER_MODE == "SS":
                    tmp_is_best = rb_val_ious.is_best_max   # mIoU 기준
                ...
                if tmp_is_best:
                    ...
                    if not os.path.exists(tmp_path):
                        os.makedirs(tmp_path)
                    torch.save(model.state_dict(), tmp_path + str(i_epoch + 1) +'_model_state_dict.pt')
                else:
            ...
            if flag_error != 0:
                dict_2_txt(in_file_path = PATH_OUT_LOG + "error/"
                          ,in_file_name = "log_error_" + i_mode + "_" + str(i_epoch + 1) + "_" + str(flag_error) + ".csv"
                          ,in_dict = dict_log_error
                          )
