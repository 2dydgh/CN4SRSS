...
if __name__ == '__main__':
    ...
    #--- args_options.apply_cj
    if args_options.apply_cj:
        _str = "use color jitter settings in _opt.py"
        warnings.warn(_str)
        # pass
    else:
        _str = "don't apply color jitter data augmentation"
        warnings.warn(_str)
    ...
    #--- args_options.HP_LR_T, HP_WD_T, HP_LR_S, HP_WD_S, HP_LR_M, HP_WD_M, HP_LR_SRSS, HP_WD_SRSS
    # apply_float_if_not_None 활용
    
    #--- args_options.HP_BATCH_TRAIN
    # apply_int_if_not_None 활용
    #--- args_options.???
    ...
    if HP_DATASET_NAME == "CamVid" or HP_DATASET_NAME == "CamVid_5Fold":
        ...
        HP_BATCH_TRAIN          = apply_int_if_not_None(8, args_options.HP_BATCH_TRAIN, "HP_BATCH_TRAIN")
        ...
        HP_BATCH_TRAIN          = apply_int_if_not_None(4, args_options.HP_BATCH_TRAIN, "HP_BATCH_TRAIN")
    ...
    # 터졌음: 2e-3, 1e-3
    HP_LR_T = apply_float_if_not_None(5e-4, args_options.HP_LR_T, "HP_LR_T")
    HP_WD_T = apply_float_if_not_None(0,    args_options.HP_WD_T, "HP_WD_T")
    optimizer_t = torch.optim.Adam(model_t.parameters(), lr = HP_LR_T, weight_decay = HP_WD_T, betas= (0.9, 0.99))  # teacher
    
    # student
    HP_LR_S = apply_float_if_not_None(1e-3, args_options.HP_LR_S, "HP_LR_S")
    HP_WD_S = apply_float_if_not_None(0,    args_options.HP_WD_S, "HP_WD_S")
    ...
    if _opt_ss == list_opt_ss[0]:   # a, "D3P"
        HP_LR_M = apply_float_if_not_None(2e-3, args_options.HP_LR_M, "HP_LR_M")
        HP_WD_M = apply_float_if_not_None(1e-9, args_options.HP_WD_M, "HP_WD_M")
        
        HP_LR_SRSS = apply_float_if_not_None(2e-3, args_options.HP_LR_SRSS, "HP_LR_SRSS")
        HP_WD_SRSS = apply_float_if_not_None(1e-9, args_options.HP_WD_SRSS, "HP_WD_SRSS")
        
    elif _opt_ss == list_opt_ss[1]: # b, "DABNet"
        HP_LR_M = apply_float_if_not_None(2e-3, args_options.HP_LR_M, "HP_LR_M")
        HP_WD_M = apply_float_if_not_None(1e-9, args_options.HP_WD_M, "HP_WD_M")
        
        HP_LR_SRSS = apply_float_if_not_None(1e-3, args_options.HP_LR_SRSS, "HP_LR_SRSS")
        HP_WD_SRSS = apply_float_if_not_None(1e-9, args_options.HP_WD_SRSS, "HP_WD_SRSS")
    ...
    # ss
    optimizer_m    = torch.optim.Adam(model_m.parameters(), lr = HP_LR_M,    weight_decay = HP_WD_M,    betas= (0.9, 0.99))
    
    # srss (model_s + model_l)
    _params_l = list(model_s.parameters()) + list(model_m.parameters())
    optimizer_srss = torch.optim.Adam(_params_l, lr = HP_LR_SRSS, weight_decay = HP_WD_SRSS, betas= (0.9, 0.99))  # srss (model_s + model_l)
    ...
    update_dict_v2("", ""
                  ,"", "옵티마이저 정보 (seMantic segmentation)"
                  ,"", "optimizer: " + "torch.optim.Adam"
    ...
    update_dict_v2("", ""
                  ,"", "옵티마이저 정보 (CN4SRSS)"
                  ,"", "optimizer: " + "torch.optim.Adam"
                  ,"", "learning_rate: " + str(HP_LR_SRSS)
                  ,"", "weight decay: " + str(HP_WD_SRSS)
                  ,in_dict = dict_log_init
                  ,in_print_head = "dict_log_init"
                  )
