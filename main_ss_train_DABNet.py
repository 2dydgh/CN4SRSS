...
if __name__ == '__main__':
    ...
    # https://github.com/Reagan1311/DABNet/blob/master/train.py#L302
    if HP_DATASET_NAME == "CamVid":
        force_fix_size_hr   = (480, 360)        # DABNet official: Train code setting for CamVid     (W480, H360)
        HP_BATCH_TRAIN      = 8                 # DABNet official: 8 (paper)
        HP_CHANNEL_HYPO     = HP_LABEL_TOTAL - 1
        
    elif HP_DATASET_NAME == "MiniCity":
        force_fix_size_hr   = (1024, 512)       # DABNet official: Train code setting for Cityscapes (W1024, H512)
        HP_BATCH_TRAIN      = 8                 # DABNet official: 8 (paper)
        HP_CHANNEL_HYPO     = HP_LABEL_TOTAL - 1
    ...
    model_ss_name = "DABNet"   #DABNet
    
    model_ss = DABNet(classes=HP_CHANNEL_HYPO)
    update_dict_v2("", ""
                  ,"", "모델 정보: " + model_ss_name
                  ,"", "(github) DABNet"
                  ,"", "pretrained = False"
    ...
    #option_optimizer = "SGD"           # DABNet official -> 학습 정상적으로 수행 불가
    option_optimizer = "ADAM"
    
    if option_optimizer == "SGD":
        #<<< SGD
        HP_LR          = 4.5e-2         # DABNet official: 4.5e-2
        HP_LR_MOMENTUM = 0.9            # DABNet official: 0.9
        HP_WD          = 1e-4           # DABNet official: 1e-4
    ...
    option_scheduer = "Poly"                           # DABNet official
    ...
    if option_scheduer == "Poly":
        ...
        HP_SCHEDULER_POWER              = 0.9          # DABNet official: 0.9
    ...
    trainer_(# <patch를 통해 학습 & RGB 이미지를 생성하는 모델>#
            ...
            ,HP_AUGM_PROB_FLIP              = 50                        # DABNet official: mirror 적용
            ,HP_AUGM_PROB_CROP              = -1                        # DABNet official: Augm에 원본 이미지에 대한 random crop X
            ,HP_AUGM_PROB_ROTATE            = -1                        # DABNet official: Augm에 random rotation X
            ,HP_CJ_BRIGHTNESS               = [1,1]                     # DABNet official: Augm에 colorJitter X
            ,HP_CJ_CONTRAST                 = [1,1]                     # DABNet official: Augm에 colorJitter X
            ,HP_CJ_SATURATION               = [1,1]                     # DABNet official: Augm에 colorJitter X
            ,HP_CJ_HUE                      = [0,0]                     # DABNet official: Augm에 colorJitter X
            ,HP_AUGM_RANDOM_SCALER          = [0.75, 1.0, 1.25, 1.5, 1.75, 2.0]    # is_force_fix = True의 경우에 적용가능
