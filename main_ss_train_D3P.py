...
if __name__ == '__main__':
    ...
    # https://github.com/tensorflow/models/blob/master/research/deeplab/train.py#L134
    if HP_DATASET_NAME == "CamVid":
        force_fix_size_hr   = (513, 513)            # D3P official: (w513, h513) *논문 참고
        HP_BATCH_TRAIN      = 4                     # D3P official: 16 (paper) -> 재현 어려움 (VRAM 12GB)
    ...
    #option_optimizer = "SGD"       # D3P official: SGD -> 학습 정상적으로 수행 불가
    option_optimizer = "ADAM"
    
    if option_optimizer == "SGD":
        #<<< SGD
        HP_LR          = 0.007          # D3P official: 0.007 (train), 0.0001 (fine-tune)
        HP_LR_MOMENTUM = 0.9            # D3P official: (code) 0.9
        HP_WD          = 0.00004        # D3P official: (code) 0.00004
        ...
        #<<< ADAM
        HP_LR       = 1e-3      # D3P official: (code) 0.001
        HP_WD       = 0.00004   # D3P official: (code) 0.00004
    ...
    option_scheduer = "Poly"           # D3P official: Poly
    ...
    if option_scheduer == "Poly":
        ...
        HP_SCHEDULER_POWER              = 0.9          # D3P official: 0.9
    ...
    trainer_(# <patch를 통해 학습 & RGB 이미지를 생성하는 모델>#
            ...
            ,HP_COLOR_MAP                   = HP_COLOR_MAP
             ...
             # Patch 생성 관련 강제 Margin 추가여부 (bool, is_use_patch와 동시 사용불가, lr 이미지 관련 값은 scale factor에 비례한 값 자동 입력됨)
            ,is_force_fix                   = True                      # D3P official: True
            ...
            ,HP_AUGM_PROB_FLIP              = 50                        # D3P official: randomly left-right flipping
            ,HP_AUGM_PROB_CROP              = -1                        # D3P official: Force Fit으로 대체
            ,HP_AUGM_PROB_ROTATE            = -1                        # D3P official: 적용 안함
            ,HP_CJ_BRIGHTNESS               = [1,1]                     # D3P official: colorJitter X
            ,HP_CJ_CONTRAST                 = [1,1]                     # D3P official: colorJitter X
            ,HP_CJ_SATURATION               = [1,1]                     # D3P official: colorJitter X
            ,HP_CJ_HUE                      = [0,0]                     # D3P official: colorJitter X
            ,HP_AUGM_RANDOM_SCALER          = [0.5, 0.75, 1.0, 1.25, 1.5, 1.75, 2.0]    # D3P official: 0.5 ~ 2.0 (0.25 간격)
