...
if __name__ == '__main__':
    from DLCs.super_resolution.model_rfdn   import RFDN   # loss: torch.nn.L1Loss()
    ...
    update_dict_v2("", "loss 정보"
                  ,"", "loss: L1 Loss (Train from scratch에 해당한 경우에 사용)"
                  ,in_dict = dict_log_init
                  ,in_print_head = "dict_log_init"
                  )
    ...
    #[optimizer]------------------------
    HP_LR = 5e-4                        # RFDN official: 5 x 10^-4 (ADAM, beta1=0.9, beta2=0.999, eps=1e-08)
    ...
    option_scheduer = "Step"            # RFDN official: StepLR
    ...
    trainer_(# <patch를 통해 학습 & RGB 이미지를 생성하는 모델>#
            ...
            ,HP_EPOCH                       = HP_EPOCH
            ,HP_BATCH_TRAIN                 = 64                        # RFDN official: 64 (HR 기준 256x256 이미지에 대해 VRAM 4.1 GB 사용됨)
            ,HP_BATCH_VAL                   = 1
            ,HP_BATCH_TEST                  = 1
            ...
            ,is_force_fix                   = True
            ,force_fix_size_hr              = (256, 256)                # RFDN - official 설정 : LR 기준 64x64
