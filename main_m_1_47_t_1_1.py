# main_m_1_47_t_1_1.py

if __name__ == '__main__':
    import matplotlib
    matplotlib.use("Agg")
    
    from _private_models.model_1_47         import proposed_model, proposed_loss, proposed_loss_ss
    from utils.schedulers                   import PolyLR, Poly_Warm_Cos_LR
    from _private_models.trainer_1_1        import *
    ...
    if _opt_ss   == list_opt_ss[0]: # a
        model_l, _str = DeepLab_v3_plus(num_classes = HP_CHANNEL_HYPO, pretrained = False), "D3P (a)"
    elif _opt_ss == list_opt_ss[1]: # b
        model_l, _str = DABNet(classes=HP_CHANNEL_HYPO), "DABNet (b)"
    elif _opt_ss == list_opt_ss[2]: # c
        model_l, _str = CGNet(classes=HP_CHANNEL_HYPO, M=3, N=21), "CGNet (c)"
    ...
    criterion_ss = proposed_loss_ss(pred_classes    = HP_CHANNEL_HYPO
                                   ,ignore_index    = HP_LABEL_VOID
                                   ,alpha           = wce_alpha
                                   ,beta            = wce_beta
                                   )
    
    criterion_kd_ss = proposed_loss_ss(pred_classes    = HP_CHANNEL_HYPO
                                      ,ignore_index    = HP_LABEL_VOID
                                      ,alpha           = wce_alpha
                                      ,beta            = wce_beta
                                      )
    
    criterion_l  = proposed_loss_ss(pred_classes    = HP_CHANNEL_HYPO
                                   ,ignore_index    = HP_LABEL_VOID
                                   ,alpha           = wce_alpha
                                   ,beta            = wce_beta
                                   )
    ...
    # ss
    optimizer_ss    = torch.optim.Adam(model_l.parameters(), lr = HP_LR_SS,    weight_decay = HP_WD_SS,    betas= (0.9, 0.99))  # sr_n_ss SS
    optimizer_kd_ss = torch.optim.Adam(model_l.parameters(), lr = HP_LR_KD_SS, weight_decay = HP_WD_KD_SS, betas= (0.9, 0.99))  # kd_sr_2_ss SS
    
    # srss (model_s + model_l)
    _params_l = list(model_s.parameters()) + list(model_l.parameters())
    optimizer_l = torch.optim.Adam(_params_l, lr = HP_LR_L, weight_decay = HP_WD_L, betas= (0.9, 0.99))  # srss (model_s + model_l)
    ...
    if optimizer_kd_ss is not None:
        update_dict_v2("", ""
                      ,"", "옵티마이저 정보 (kd_sr_2_ss SS)"
                      ,"", "optimizer: " + "torch.optim.Adam"
                      ,"", "learning_rate: " + str(HP_LR_KD_SS)
                      ,"", "weight decay: " + str(HP_WD_KD_SS)
                      ,in_dict = dict_log_init
                      ,in_print_head = "dict_log_init"
                      )
    ...
    #[scheduler]----------------------------------------------
    HP_EPOCH                        = 1002          # 학습 종료 epoch
    HP_SCHEDULER_TOTAL_EPOCH        = 1000         # option for Poly
    ...
    if optimizer_ss is not None:
        scheduler_ss = Poly_Warm_Cos_LR(optimizer_ss
    ...
    if optimizer_kd_ss is not None:
        scheduler_kd_ss = Poly_Warm_Cos_LR(optimizer_kd_ss
                                          ,warm_up_steps=HP_SCHEDULER_WARM_STEPS
                                          ,T_max=HP_SCHEDULER_T_MAX, eta_min=HP_SCHEDULER_ETA_MIN, style=HP_SCHEDULER_STYLE
                                          ,power=HP_SCHEDULER_POWER, max_epoch=HP_SCHEDULER_TOTAL_EPOCH
                                          )
        update_dict_v2("", ""
                      ,"", "스케쥴러 정보 (kd_sr_2_ss SS)"
                      ,"", "업데이트 간격: epoch" 
                      ,"", "scheduler: Poly_Warm_Cos_LR"
                      ,"", "Power: "   + str(HP_SCHEDULER_POWER)
                      ,"", "Warm up: " + str(HP_SCHEDULER_WARM_STEPS)
        ...
        scheduler_kd_ss = None
    
    
    if optimizer_l is not None:
        scheduler_l = Poly_Warm_Cos_LR(optimizer_l
                                      ,warm_up_steps=HP_SCHEDULER_WARM_STEPS
                                      ,T_max=HP_SCHEDULER_T_MAX, eta_min=HP_SCHEDULER_ETA_MIN, style=HP_SCHEDULER_STYLE
                                      ,power=HP_SCHEDULER_POWER, max_epoch=HP_SCHEDULER_TOTAL_EPOCH
                                      )
    ...
    trainer_(# <patch를 통해 학습 & RGB 이미지를 생성하는 모델>#
            ...
            ,criterion_s                    = criterion_s
             ...
             # (sr_n_ss SS) model, optimizer, scheduler, loss
            ,model_l                        = model_l
            ,optimizer_ss                   = optimizer_ss
            ,scheduler_ss                   = scheduler_ss
            ,criterion_ss                   = criterion_ss
            
             # (kd_sr_2_ss SS) model, optimizer, scheduler, loss
            # ,model_l                      = model_l
            ,optimizer_kd_ss                = optimizer_kd_ss
            ,scheduler_kd_ss                = scheduler_kd_ss
            ,criterion_kd_ss                = criterion_kd_ss
            ,HP_MIXUP_A                     = HP_MIXUP_A
            
             # (SRSS) model, optimizer, scheduler, loss
            # ,model_l                      = model_l
