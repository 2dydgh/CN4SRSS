...
if __name__ == '__main__':
    from DLCs.super_resolution.model_bsrn   import BSRN   # loss: torch.nn.L1Loss()
    ...
    criterion_sr = torch.nn.L1Loss()                # BSRN official: L1 Loss
    ...
    #[optimizer]------------------------
    HP_LR = 1e-3                    # BSRN official: 1 x 10^-3 (ADAM, beta1=0.9, beta2=0.999)
    #                                 BSRN official: weight decay 사용 안됨
    ...
    #[scheduler]----------------------------------------------
    # BSRN 공식: cosine learning rate decay
    HP_EPOCH                     = 3002         # 비교실험용 공통값
    HP_SCHEDULER_TOTAL_EPOCH     = 3000         # 스케줄러 종료 시점
    HP_ETA_MIN                   = 1e-7         # BSRN official: code에서 float 1e-7로 설정값 확인
    HP_SCHEDULER_UPDATE_INTERVAL = "epoch"      # 스케줄러 갱신 간격 ("epoch" 또는 "batch")
    
    # CosineDecayLR -> "The initial learning rate is set to 1 × 10−3 with cosine learning rate decay."
    # 공식코드 참고하여 재구현 (Paper에 스케쥴러 update 간격은 언급 없음 -> epoch 단위로 설정)
    scheduler = CosineDecayLR(optimizer = optimizer
                             ,eta_min   = HP_ETA_MIN
    ...
    update_dict_v2("", ""
                  ...
                  ,"", "scheduler: " + "CosineDecayLR (재구현)"
                  ,"", "eta min: " + str(HP_ETA_MIN)
                  ,in_dict = dict_log_init
                  ,in_print_head = "dict_log_init"
                  )
    ...
    trainer_(# <patch를 통해 학습 & RGB 이미지를 생성하는 모델>#
            ...
            ,HP_EPOCH                       = HP_EPOCH
            ,HP_BATCH_TRAIN                 = 64             # BSRN official: 64 (HR 기준 192x192 이미지에 대해 VRAM 5.8GB 사용됨)
            ,HP_BATCH_VAL                   = 1
            ,HP_BATCH_TEST                  = 1
            ...
            ,is_force_fix                   = True
            ,force_fix_size_hr              = (192, 192)                # BSRN - official 설정 : LR 기준 48x48
            ...
            ,HP_AUGM_LITE                   = True                      # SR 모드에서 lite 모드 augm 시행여부
