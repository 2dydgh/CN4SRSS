...
v 1.55
    proposed_loss_ss 에서
        edge 를 기반으로 수행하는 loss_b 는 도움이 안되는거 같음
            --> 제거함
        m 1.48 부터 _weight 가 적용되지 않던 문제가 확인됨
            --> 학습에 반영되도록 수정
    ...
    def forward(self, in_pred, in_ans, *args):
        ...
        if in_teacher is None:
            ...
            if _feat_count is None:
                ...
                return self.alpha * ((1 - _alpha) * self.loss_l1(in_pred_sr, in_ans) + _alpha * (loss_kd + loss_feat / _feat_count))



class proposed_loss_ss(nn.Module):
    def __init__(self, is_amp=True, pred_classes=None, ignore_index=-100, beta=1.0):
    ...
    def forward(self, in_pred, in_ans):
        ...
        return self.beta * torch.nn.functional.cross_entropy(torch.log(in_pred + self.eps)
                                                            ,in_ans
                                                            ,weight=_weight
