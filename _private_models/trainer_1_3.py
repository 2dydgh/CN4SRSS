...
v 0.7
    매 Train 시작마다 ETA 출력하도록 수정 (Estimated time Arrival)
    sr_n_ss 추가
...
def one_epoch_teacher(**kargs):
    # One epoch for Teacher (SR network)
    ...
    for dataloader_items in dataloader_input:
        ...
        if i_mode == "train":
            with torch.cuda.amp.autocast(enabled=True):
                tensor_out_sr_raw = model(dl_ts_img_lr)
                loss = criterion(tensor_out_sr_raw, dl_ts_img_hr)
        ...
        dict_rb[i_mode]["loss"].add_item(loss.item())
        ...
        for i_image in range(current_batch_size):
            ...
            tmp_str_contents = (str(i_batch + 1) + "," + dl_str_file_name[i_image] + "," + str(loss.item()) 
            ...
            if is_pil_needed and i_image < 2:
                ...
                if len(list_mp_buffer) >= BUFFER_SIZE:
                    plts_saver_sr(list_mp_buffer, is_best=(i_mode=="test"), no_employ=(employ_threshold >= len(list_mp_buffer)))
                    try:
                        del list_mp_buffer
                        list_mp_buffer = []
                    except:
                ...
                list_mp_buffer.append((# 0 (model name)
                                       ...
                                       "SR_B"
                                       # 1 ~ 3 (pils)
                                      ,dl_pil_img_hr[i_image], dl_pil_img_lr[i_image], list_out_pil_sr[i_image]
                                       # 4 ~ 6 (sub title)
                                      ,"HR Image", "LR Image", "SR Image"
                                       # 7 (path for plt)
                                      ,PATH_OUT_IMAGE + i_mode + "/gt_" + str(i_epoch + 1)
                                       # 8 (path for SR pil)
                                      ,PATH_OUT_IMAGE + i_mode + "/_SR_Images/gt_" + str(i_epoch + 1)
                                       # 9 (plt title)
                                      ,plt_title
                                       # 10 (file name)
                                      ,tmp_file_name
    ...
    if i_mode == "train":
        dict_rb[i_mode]["lr"].add_item(optimizer.param_groups[0]['lr'])
    ...
    dict_2_txt_v2(in_file_path = PATH_OUT_LOG + i_mode + "/"
                 ,in_file_name = "gt_log_epoch_" + i_mode + "_" + str(i_epoch + 1) + ".csv"
                 ,in_dict_dict = dict_dict_log_epoch
                 ,in_dict_key = i_mode
                 )
    ...
    dict_2_txt_v2(in_file_path = PATH_OUT_LOG
                 ,in_file_name = "gt_log_total_" + i_mode + ".csv"
                 ,in_dict_dict = dict_dict_log_total
                 ,in_dict_key = i_mode
                 )
    ...
    if i_mode == "train" and (i_epoch + 1) % save_interval == 0:
        ...
        torch.save({'epoch': (i_epoch + 1)                           # (int) 중단 시점 epoch 값
                  ...
                  ,tmp_path + str(i_epoch + 1) +'_teacher_check_point.tar'
                  )
    
    if i_mode == "val":
        tmp_is_best = dict_rb[i_mode]["psnr"].is_best_max # PSNR 기준
        ...
        if tmp_is_best:
            ...
            if not os.path.exists(tmp_path):
                os.makedirs(tmp_path)
            torch.save(model.state_dict(), tmp_path + "GT_" +str(i_epoch + 1) +'_msd.pt')
        else:
...
#=== End of one_epoch_teacher



def one_epoch_student(**kargs):
    # One epoch for Student (SR network)
    # TRAINER_MODE        = kargs['TRAINER_MODE']
    list_mode           = kargs['list_mode']
    i_mode              = kargs['i_mode']
    HP_EPOCH            = kargs['HP_EPOCH']
    ...
    for dataloader_items in dataloader_input:
        ...
        with torch.no_grad():
            kd_ts_img_sr_raw = model_t(dl_ts_img_lr.clone().detach())
            # if isinstance(kd_ts_img_sr_raw, list):
                # kd_ts_img_sr = kd_ts_img_sr_raw[0]
            # else:
                # kd_ts_img_sr = kd_ts_img_sr_raw
        ...
        if i_mode == "train":
            with torch.cuda.amp.autocast(enabled=True):
                tensor_out_sr_raw = model(dl_ts_img_lr)
                loss = criterion(tensor_out_sr_raw, dl_ts_img_hr, kd_ts_img_sr_raw)
                if isinstance(tensor_out_sr_raw, list):
                    # model 설계시, 첫 output을 SR 이미지로 설정할 것
                    tensor_out_sr = tensor_out_sr_raw[0]
                else:
        ...
        for i_image in range(current_batch_size):
            ...
            if is_pil_needed and i_image < 2:
                ...
                list_mp_buffer.append((# 0 (model name)
                                      ...
                                      ,"HR Image", "LR Image", "SR Image"
                                       # 7 (path for plt)
                                      ,PATH_OUT_IMAGE + i_mode + "/kd_s_" + str(i_epoch + 1)
                                       # 8 (path for SR pil)
                                      ,PATH_OUT_IMAGE + i_mode + "/_SR_Images/kd_s_" + str(i_epoch + 1)
                                       # 9 (plt title)
                                      ,plt_title
                                       # 10 (file name)
                                      ,tmp_file_name
    ...
    # log 기록 업데이트 (epoch 단위)
    dict_2_txt_v2(in_file_path = PATH_OUT_LOG + i_mode + "/"
                 ,in_file_name = "kd_log_epoch_" + i_mode + "_" + str(i_epoch + 1) + ".csv"
                 ,in_dict_dict = dict_dict_log_epoch
                 ,in_dict_key = i_mode
                 )
    ...
    # log 기록 업데이트 (학습 전체 단위)
    dict_2_txt_v2(in_file_path = PATH_OUT_LOG
                 ,in_file_name = "kd_log_total_" + i_mode + ".csv"
                 ,in_dict_dict = dict_dict_log_total
                 ,in_dict_key = i_mode
                 )
    ...
    if i_mode == "train" and (i_epoch + 1) % save_interval == 0:
        ...
        torch.save({'epoch': (i_epoch + 1)                           # (int) 중단 시점 epoch 값
                  ...
                  ,tmp_path + str(i_epoch + 1) +'_student_check_point.tar'
                  )
    ...
    if i_mode == "val":
        ...
        if tmp_is_best:
            ...
            if not os.path.exists(tmp_path):
                os.makedirs(tmp_path)
            torch.save(model.state_dict(), tmp_path + "KD_" + str(i_epoch + 1) +'_s_msd.pt')
            
            try:
                # teacher's model state dict도 저장
                torch.save(model_t.state_dict(), tmp_path + "KD_" + str(i_epoch + 1) +'_t_msd.pt')
            except:
                pass
...
#=== End of one_epoch_student



def one_epoch_srss(**kargs):
    # One epoch for Student (SR network)
    # TRAINER_MODE        = kargs['TRAINER_MODE']
    list_mode           = kargs['list_mode']
    i_mode              = kargs['i_mode']
    ...
    model               = kargs['model']            # image to label generator (sem. seg.)
    optimizer           = kargs['optimizer']        # model_s + model_l
    criterion           = kargs['criterion']
    scheduler           = kargs['scheduler']
    ...
    for dataloader_items in dataloader_input:
        ...
        if i_mode == "train":
            with torch.cuda.amp.autocast(enabled=True):
                ...
                tensor_out_seg = model(tensor_out_sr)
            ...
            if is_pil_needed and i_image < 2:
                ...
                list_mp_buffer.append((# 0 (model name)
                                      ...
                                      ,"HR Image", "Label Answer", "Predicted", "HR Image", "LR Image", "SR Image"
                                       # 13 (path plt)
                                      ,PATH_OUT_IMAGE + i_mode + "/srss_" + str(i_epoch + 1)
                                       # 14 ~ 17 (data CF)
                                      ,None, None, None, None
                                       # 18 (path CF)
                                      ,PATH_OUT_IMAGE + i_mode + "/" + str(i_epoch + 1)
                                       # 19 (path pil)
                                      ,PATH_OUT_IMAGE + i_mode + "/_SRSS_Images/srss_" + str(i_epoch + 1)
                                       # 20 (plt title)
                                      ,plt_title
                                       # 21 (file name)
                                      ,tmp_file_name
    ...
    # log 기록 업데이트 (epoch 단위)
    dict_2_txt_v2(in_file_path = PATH_OUT_LOG + i_mode + "/"
                 ,in_file_name = "srss_log_epoch_" + i_mode + "_" + str(i_epoch + 1) + ".csv"
                 ,in_dict_dict = dict_dict_log_epoch
                 ,in_dict_key = i_mode
                 )
    ...
    # log 기록 업데이트 (학습 전체 단위)
    dict_2_txt_v2(in_file_path = PATH_OUT_LOG
                 ,in_file_name = "srss_log_total_" + i_mode + ".csv"
                 ,in_dict_dict = dict_dict_log_total
                 ,in_dict_key = i_mode
                 )
    ...
    if i_mode == "train" and (i_epoch + 1) % save_interval == 0:
        ...
        torch.save({'epoch': (i_epoch + 1)                           # (int) 중단 시점 epoch 값
                  ...
                  ,tmp_path + str(i_epoch + 1) +'_label_student_check_point.tar'
                  )
    ...
    if i_mode == "val":
        ...
        if tmp_is_best:
            ...
            if not os.path.exists(tmp_path):
                os.makedirs(tmp_path)
            torch.save(model.state_dict(), tmp_path + "SRSS_" + str(i_epoch + 1) +'_ss_msd.pt')
            
            try:
                # student's model state dict도 저장
                torch.save(model_s.state_dict(), tmp_path + "SRSS_" + str(i_epoch + 1) +'_sr_msd.pt')
                torch.save(model_t.state_dict(), tmp_path + "SRSS_" + str(i_epoch + 1) +'_t_msd.pt')
            except:
                pass
...
def one_epoch_sr_n_ss(**kargs):
    # backward gradient 끊어줌
    # backward gradient 끊기 싫으면 srss 쓰기
    ...
    model_t             = kargs['model_t']          # Teacher
    optimizer_t         = kargs['optimizer_t']      # Teacher
    criterion_t         = kargs['criterion_t']      # Teacher
    scheduler_t         = kargs['scheduler_t']      # Teacher
    
    # model_s             = kargs['model_s']          # Student (학습에 관여 안함) -> 함수 밖에서 CPU로 보내기
    
    model               = kargs['model']            # image to label generator (sem. seg.)
    optimizer           = kargs['optimizer']        # model_l 만
    criterion           = kargs['criterion']        # model_l 만
    scheduler           = kargs['scheduler']        # model_l 만
    ...
    dict_rb             = kargs['dict_rb']    # record box (ss)
    ...
    for dataloader_items in dataloader_input:
        ...
        if i_mode == "train":
            with torch.cuda.amp.autocast(enabled=True):
                tensor_out_sr_raw = model_t(dl_ts_img_lr)
                if isinstance(tensor_out_sr_raw, list):
                    tensor_out_sr = tensor_out_sr_raw[0] # model 설계시, 첫 output을 SR 이미지로 설정할 것
                else:
                    tensor_out_sr = tensor_out_sr_raw
                
                tensor_out_sr_copy = tensor_out_sr.clone().detach()
                tensor_out_sr_copy = tensor_out_sr_copy.requires_grad_(True)
                
                loss_t = criterion_t(tensor_out_sr_raw, dl_ts_img_hr)
            ...
            with torch.cuda.amp.autocast(enabled=True):
                tensor_out_seg = model(tensor_out_sr_copy)
        ...
        for i_image in range(current_batch_size):
            with torch.no_grad():
                ...
                dict_rb_t[i_mode]["psnr"].add_item(out_psnr)
                dict_rb_t[i_mode]["ssim"].add_item(out_ssim)
            ...
            tmp_str_contents = (str(i_batch + 1) + "," + dl_str_file_name[i_image] + "," + str(loss_t.item()) + "," + str(loss.item()) 
                               +"," + str(out_psnr) + "," + str(out_ssim)
                               +"," + str(tmp_pa) + "," + str(tmp_ca) +"," + str(tmp_miou) + tmp_ious
                               )
            ...
            if is_pil_needed and i_image < 2:
                ...
                list_mp_buffer.append((# 0 (model name)
                                      ...
                                      ,"HR Image", "Label Answer", "Predicted", "HR Image", "LR Image", "SR Image"
                                       # 13 (path plt)
                                      ,PATH_OUT_IMAGE + i_mode + "/sr_n_ss_" + str(i_epoch + 1)
                                       # 14 ~ 17 (data CF)
                                      ,None, None, None, None
                                       # 18 (path CF)
                                      ,PATH_OUT_IMAGE + i_mode + "/" + str(i_epoch + 1)
                                       # 19 (path pil)
                                      ,PATH_OUT_IMAGE + i_mode + "/_SR_n_SS_Images/sr_n_ss_" + str(i_epoch + 1)
                                       # 20 (plt title)
                                      ,plt_title
                                       # 21 (file name)
                                      ,tmp_file_name
    ...
    for i_key in ["psnr", "ssim"]:
        tmp_str_contents += "," + str(dict_rb_t[i_mode][i_key].update_epoch(is_return = True, path = PATH_OUT_LOG
                                                                           ,is_print_sub = True ,is_update_graph = True
                                                                           )
                                     )
    ...
    for i_key in ["psnr", "ssim"]:
        dict_rb[i_mode][i_key].update_epoch(is_return = False, path = PATH_OUT_LOG ,is_print_sub = False ,is_update_graph = False)
    ...
    # log 기록 업데이트 (epoch 단위)
    dict_2_txt_v2(in_file_path = PATH_OUT_LOG + i_mode + "/"
                 ,in_file_name = "sr_n_ss_log_epoch_" + i_mode + "_" + str(i_epoch + 1) + ".csv"
                 ,in_dict_dict = dict_dict_log_epoch
                 ,in_dict_key = i_mode
                 )
    ...
    # log 기록 업데이트 (학습 전체 단위)
    dict_2_txt_v2(in_file_path = PATH_OUT_LOG
                 ,in_file_name = "sr_n_ss_log_total_" + i_mode + ".csv"
                 ,in_dict_dict = dict_dict_log_total
                 ,in_dict_key = i_mode
                 )
    ...
    if i_mode == "train" and (i_epoch + 1) % save_interval == 0:
        ...
        torch.save({'epoch': (i_epoch + 1)                              # (int) 중단 시점 epoch 값
                  ...
                  ,tmp_path + str(i_epoch + 1) +'_sr_n_ss_check_point.tar'
                  )
    
    if i_mode == "val":
        tmp_is_best = dict_rb[i_mode]["psnr"].is_best_max or dict_rb[i_mode]["ious"].is_best_max # PSNR 기준 or mIoU 기준
        ...
        if tmp_is_best:
            ...
            if not os.path.exists(tmp_path):
                os.makedirs(tmp_path)
            torch.save(model.state_dict(), tmp_path + "SR_n_SS_" + str(i_epoch + 1) +'_ss_msd.pt')
            torch.save(model_t.state_dict(), tmp_path + "SR_n_SS_" + str(i_epoch + 1) +'_t_msd.pt')
            
            # try:
                # # student's model state dict도 저장
                # torch.save(model_s.state_dict(), tmp_path + "SR_n_SS_" + str(i_epoch + 1) +'_s_msd.pt')
            # except:
                # pass
...
def one_epoch_kd_sr_2_ss(**kargs):
    # One epoch for Student (SR network) and KD_SS (SS network)
    # TRAINER_MODE        = kargs['TRAINER_MODE']
    list_mode           = kargs['list_mode']
    i_mode              = kargs['i_mode']
    ...
    model_ss            = kargs['model_ss']         # SS network
    optimizer_ss        = kargs['optimizer_ss']     # only SS network
    criterion_ss        = kargs['criterion_ss']
    scheduler_ss        = kargs['scheduler_ss']
    ...
    # dict_rb             = kargs['dict_rb']  # record box
    dict_rb_sr          = kargs['dict_rb_sr']  # record box SR (S)
    dict_rb_ss          = kargs['dict_rb_ss']  # record box SS
    ...
    for dataloader_items in dataloader_input:
        ...
        if i_mode == "train":
            ...
            with torch.cuda.amp.autocast(enabled=True):
                ...
                tensor_out_sr_mixup = kd_ts_img_sr * HP_MIXUP_A + tensor_out_sr_copy * (1 - HP_MIXUP_A)
                tensor_out_sr_mixup = tensor_out_sr_mixup.requires_grad_(True)
                tensor_out_seg = model_ss(tensor_out_sr_mixup)
                
                # label 예측결과 softmax 시행
                tensor_out_seg_softmax = F.softmax(tensor_out_seg, dim = 1)
                loss_ss = criterion_ss(tensor_out_seg_softmax, _dl_ts_lab_hr_gray)
                
                # softmax 값을 바탕으로 label image 형태로 tensor 생성 (형태 변경 4D [B, C, H, W] -> 3D [B, H, W]) -> 이미지 생성에 사용됨
                tensor_out_seg_label = torch.argmax(tensor_out_seg_softmax.clone().detach().requires_grad_(False), dim = 1)
                
            amp_scaler.scale(loss_ss).backward(retain_graph=False)
            amp_scaler.step(optimizer_ss)
            amp_scaler.update()
            optimizer_ss.zero_grad()
        ...
        dict_rb_sr[i_mode]["loss"].add_item(loss.item())
        dict_rb_ss[i_mode]["loss"].add_item(loss_ss.item())
        ...
        if is_pil_needed:
            list_out_pil_sr         = tensor_2_list_pils_v1(in_tensor=tensor_out_sr,        is_label=False, is_resized=False)
            list_out_pil_sr_mixup   = tensor_2_list_pils_v1(in_tensor=tensor_out_sr_mixup,  is_label=False, is_resized=False)
            list_out_pil_label      = tensor_2_list_pils_v1(in_tensor=tensor_out_seg_label, is_label=True, is_resized=False)
        ...
        for i_image in range(current_batch_size):
            with torch.no_grad():
                ...
                dict_rb_sr[i_mode]["psnr"].add_item(out_psnr)
                dict_rb_sr[i_mode]["ssim"].add_item(out_ssim)
                
                dict_rb_ss[i_mode]["psnr"].add_item(-9.0) # dummy
                dict_rb_ss[i_mode]["ssim"].add_item(-9.0) # dummy
            ...
            dict_rb_sr[i_mode]["pa"].add_item(tmp_pa)
            dict_rb_sr[i_mode]["ca"].add_item(tmp_ca)
            dict_rb_sr[i_mode]["ious"].add_item(dict_ious)
            ...
            dict_rb_ss[i_mode]["pa"].add_item(tmp_pa)
            dict_rb_ss[i_mode]["ca"].add_item(tmp_ca)
            dict_rb_ss[i_mode]["ious"].add_item(dict_ious)
            
            tmp_ious = ""
            for i_key in dict_ious:
                tmp_ious += "," + dict_ious[i_key]
            
            tmp_str_contents = (str(i_batch + 1) + "," + dl_str_file_name[i_image] + "," + str(loss.item()) + "," + str(loss_ss.item())
                               +"," + str(out_psnr) + "," + str(out_ssim)
                               +"," + str(tmp_pa) + "," + str(tmp_ca) +"," + str(tmp_miou) + tmp_ious
                               )
            ...
            if is_pil_needed and i_image < 2:
                ...
                list_mp_buffer.append((# 0 (model name)
                                      ...
                                      ,dl_pil_img_hr[i_image], label_2_RGB(dl_pil_lab_hr[i_image], HP_COLOR_MAP)
                                      ,label_2_RGB(list_out_pil_label[i_image], HP_COLOR_MAP), dl_pil_img_lr[i_image]
                                      ,list_out_pil_sr[i_image], list_out_pil_sr_mixup[i_image]
                                       # 7 ~ 12 (sub title)
                                      ,"HR Image", "Label Answer", "Predicted", "LR Image", "SR (s) Image", "SR (mixup) Image"
                                       # 13 (path plt)
                                      ,PATH_OUT_IMAGE + i_mode + "/kd_sr_2_ss_" + str(i_epoch + 1)
                                       # 14 ~ 17 (data CF)
                                      ,None, None, None, None
                                       # 18 (path CF)
                                      ,PATH_OUT_IMAGE + i_mode + "/" + str(i_epoch + 1)
                                       # 19 (path pil)
                                      ,PATH_OUT_IMAGE + i_mode + "/_KD_SR_2_SS_Images/kd_sr_2_ss_" + str(i_epoch + 1)
                                       # 20 (plt title)
                                      ,plt_title
                                       # 21 (file name)
                                      ,tmp_file_name
        ...
        for i_key in ["loss", "psnr", "ssim", "pa", "ca", "ious"]:
            dict_rb_sr[i_mode][i_key].update_batch()
            dict_rb_ss[i_mode][i_key].update_batch()
    ...
    tmp_str_contents  =       str(dict_rb_sr[i_mode]["loss"].update_epoch(is_return = True, path = PATH_OUT_LOG, is_print_sub = True))
    tmp_str_contents += "," + str(dict_rb_ss[i_mode]["loss"].update_epoch(is_return = True, path = PATH_OUT_LOG, is_print_sub = True))
    
    for i_key in ["psnr", "ssim"]:
        tmp_str_contents += "," + str(dict_rb_sr[i_mode][i_key].update_epoch(is_return = True, path = PATH_OUT_LOG
                                                                            ,is_print_sub = True ,is_update_graph = True
                                                                            )
                                     )
    
    for i_key in ["pa", "ca", "ious"]:
        dict_rb_sr[i_mode][i_key].update_epoch(is_return = False, path = PATH_OUT_LOG, is_print_sub = False ,is_update_graph = False)
    
    for i_key in ["psnr", "ssim"]:
        dict_rb_ss[i_mode][i_key].update_epoch(is_return = False, path = PATH_OUT_LOG ,is_print_sub = False ,is_update_graph = False)
    
    for i_key in ["pa", "ca", "ious"]:
        tmp_str_contents += "," + str(dict_rb_ss[i_mode][i_key].update_epoch(is_return = True, path = PATH_OUT_LOG
                                                                            ,is_print_sub = True ,is_update_graph = True
                                                                            )
                                     )
    ...
    if i_mode == "train":
        # SR (s)
        dict_rb_sr[i_mode]["lr"].add_item(optimizer.param_groups[0]['lr'])
        dict_rb_sr[i_mode]["lr"].update_batch()
        # 스케쥴러 갱신
        scheduler.step()
        print("scheduler.step()")
        dict_rb_sr[i_mode]["lr"].update_epoch(is_return = False, path = PATH_OUT_LOG, is_print_sub = True)
        
        # SS
        dict_rb_ss[i_mode]["lr"].add_item(optimizer_ss.param_groups[0]['lr'])
        dict_rb_ss[i_mode]["lr"].update_batch()
        # 스케쥴러 갱신
        scheduler_ss.step()
        print("scheduler_ss.step()")
        dict_rb_ss[i_mode]["lr"].update_epoch(is_return = False, path = PATH_OUT_LOG, is_print_sub = True)
    ...
    if i_mode == "train" and (i_epoch + 1) % save_interval == 0:
        ...
        torch.save({'epoch': (i_epoch + 1)                                  # (int) 중단 시점 epoch 값
                   ,'model_state_dict': model.state_dict()                  # (state_dict) model.state_dict()
                   ,'model_t_state_dict': model_t.state_dict()              # (state_dict) model_t.state_dict()
                   ,'model_ss_state_dict': model_ss.state_dict()            # (state_dict) model_ss.state_dict()
                   ,'optimizer_state_dict': optimizer.state_dict()          # (state_dict) optimizer.state_dict()
                   ,'scheduler_state_dict': scheduler.state_dict()          # (state_dict) scheduler.state_dict()
                   ,'optimizer_ss_state_dict': optimizer_ss.state_dict()    # (state_dict) optimizer.state_dict()
                   ,'scheduler_ss_state_dict': scheduler_ss.state_dict()    # (state_dict) scheduler.state_dict()
                   }
                  ,tmp_path + str(i_epoch + 1) +'_kd_sr_2_ss_check_point.tar'
                  )
    
    if i_mode == "val":
        # tmp_is_best = dict_rb[i_mode]["psnr"].is_best_max # PSNR 기준#@@@
        tmp_is_best = dict_rb_sr[i_mode]["psnr"].is_best_max or dict_rb_ss[i_mode]["ious"].is_best_max # PSNR 기준 or mIoU 기준
        ...
        if tmp_is_best:
            ...
            # teacher's model state dict
            torch.save(model_t.state_dict(),  tmp_path + "KD_" + str(i_epoch + 1) +'_sr_t_msd.pt')
            # student's model state dict
            torch.save(model.state_dict(),    tmp_path + "KD_" + str(i_epoch + 1) +'_sr_s_msd.pt')
            # ss's model state dict
            torch.save(model_ss.state_dict(), tmp_path + "KD_" + str(i_epoch + 1) +'_ss_msd.pt')
...
def trainer_(**kargs):
    ...
    for i_epoch in range(HP_EPOCH):
        ...
        for i_mode in list_mode:
            ...
            is_best =one_epoch_gt_kd_sr_2_ss(CALC_WITH_LOGIT = CALC_WITH_LOGIT
