...
class fixed_kernel(nn.Module):
    ...
    def __init__(self, k_type=None, in_c=3, out_c=3, device=None):
        ...
        # k_weight를 [out_channels, in_channels/groups, height, width] 형태로 변경
        k_weight = k_weight.unsqueeze(dim=0).unsqueeze(dim=0)  # 4D 텐서로 변환: [1, 1, height, width]
...
class model_I2F(nn.Module):
    ...
    def \
            __init__(self, in_c, mid_c, basic_blocks):
...
class proposed_loss(nn.Module):
    ...
    def kd_fakd(self, in_feat):
        ...
        _B, _C, _H, _W = in_feat.shape  # 특성 맵 변형
        in_feat = in_feat.view(_B, _C, -1)  # -> 각 채널에 대해 2차원 공간정보를 1차원 벡터로 평탄화한다.
        # 각 채널의 특성 벡터를 정규화 -> 유사도 계산에서 크기의 영향을 줄이는 효과
        norm_fm = in_feat / (torch.sqrt(torch.sum(torch.pow(in_feat,2), 1)).unsqueeze(1).expand(in_feat.shape) + 1e-8)
        # 유사도 행렬 계산 부분 -> 두 특성 백터 간의 내적으로, 높은 갚일 수록 두    벡터 간의 유사성이 높음을 의미한다.
        sa = norm_fm.transpose(1,2).bmm(norm_fm)
        return sa.unsqueeze(1)  # 유사도 행렬의 차원 추가
...
class proposed_loss_ss(nn.Module):
    ...
    def calc_weight(self, in_ans):
        ...
        # v 1.47 방식 폐기
        # v 1.44 수식을 one-hot 없이 사용 하게 재구현
        _B, _, _ = _ans.shape
...
class proposed_loss_srss(nn.Module):  # SR과 SS의 작업을 위한 결합 손실 함수
    def __init__(self, is_amp=True, pred_classes=None, ignore_index=-100, alpha=1.0, beta=1.0):  # 초기화 함수
        ...
        self.pred_classes   = pred_classes  
        self.ignore_index   = ignore_index  # 일반적으로 배경이나 계산에서 제외할 특정 클래스를 나타내는데 사용
        self.alpha          = alpha     # 가중치 기준값
        self.beta           = beta      # 표준편차 반영 가중치
    ...
    def calc_weight(self, in_ans):  # segmentation에서 사용되는 클래스의 가중치를 계산하는 데 사용된다.
                                    # 이 가중치는 클래스의 불균형을 처리하기 위해 설계되었다.
        # 1. 입력 텐서 준비
        _ans  = in_ans.clone().detach()
        ...
        # 2. 텐서 변형
        _B, _, _ = _ans.shape
        _ans = _ans.view([-1])  # (B, H, W) -> (BHW)  (B, H, W) 형태의 텐서를 .view([-1])를 사용하여 1차원 텐서로 변형
                                        # 결과적으로, 모든 픽셀 라벨이 단일 벡터 내에 선형적으로 배열됩니다.
        _bin = torch.bincount(_ans, minlength=self.pred_classes)[:self.pred_classes] # (BHW) -> (Index except void)
        _norm = 1 - torch.nn.functional.normalize(_bin.float(), p=1, dim=0)
