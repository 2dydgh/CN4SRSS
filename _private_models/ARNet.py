...
print("Model: CN4SRSS paper - fix 1")
# loss stage 조정
# epoch             stage
# 1                 3
# 2 ~               1
...
class Encoder_Decoder(nn.Module):
    def __init__(self, n_feat, reduction=8, bias=False, act=nn.PReLU(), groups=1, _receive_prev=True):
        ...
        self.layer_init = nn.Sequential(CCA(n_feat=n_feat, k_size=1, reduction=reduction, bias=bias, groups=groups)
                                        , CAB(n_feat=n_feat, k_size=3, reduction=reduction, bias=bias, act=act,
                                              groups=groups)
                                        )
    ...
    def forward(self, x_init, prev=None):
        ...
        if prev is None:
            ...
            y_deep = self.layer_deep(y_down  # n_feat
                                     + self.insert_down(
                F.interpolate(prev_down, size=(_h, _w), mode='bilinear', align_corners=True))
                                     )
            ...
            y_dec = self.layer_dec(y_up  # n_feat
                                   + self.insert_up(
                F.interpolate(prev_up, size=(_h, _w), mode='bilinear', align_corners=True))
                                   )
...
class BasicBlock(nn.Module):
    def __init__(self, n_feat, reduction, act=nn.PReLU(), bias=False, groups=1, is_head=False, is_tail=False, scale=1):
        ...
        if is_head:
            ...
            self.BB_ED = Encoder_Decoder(n_feat=n_feat // 2, reduction=reduction // 2, bias=bias, act=act,
                                         groups=groups, _receive_prev=False)
        else:
            self.BB_ED = Encoder_Decoder(n_feat=n_feat // 2, reduction=reduction // 2, bias=bias, act=act,
                                         groups=groups, _receive_prev=True)
...
class model_proposed_SR(nn.Module):
    def __init__(self, in_c=3, out_c_img=3, n_feat=36, reduction=6, act=nn.GELU(), bias=False, groups=1, upscale=4, front_freeze=10, front_melt=15):
        super(model_proposed_SR, self).__init__()

        self.out_c_img = out_c_img
        self.n_feat = n_feat

        # BasicBlock Layers for SR
        ...
        # Switch-related variables for freezing and melting front layers
        self._prev_mode = False  # Track the training/evaluation switch
        self._curr_epoch = 0     # Current epoch
        self._curr_stage = 1     # Current stage
    ...
    def forward(self, x_img):
        ...
        # Forward pass through SR layers
            ...
            if loss_ce_weights is not None:
                try:
                    ...
                    self.loss_ce = nn.CrossEntropyLoss(
                        weight=self.loss_ce_weights)  # loss = self.loss_ce (predict, answer)
    ...
    def calc_v6(self, pred_label, pred_sr, ans_label, ans_sr
        ...
        if LOSS_STAGE == 1:
            ...
            return torch.nan_to_num(
                self.charb_loss(F.interpolate(pred_img_1, size=(_h, _w), mode='bilinear'), ans_sr, is_AMP=is_AMP) * 0.1
