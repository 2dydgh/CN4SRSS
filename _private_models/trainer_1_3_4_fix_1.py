...
v 1.3.4
    ...
    --- fix 1
    파일 이름 head (FILE_HEAD) t_l_3_4 오타 t_1_3_4 로 수정
        log 그래프 파일 이름 head (gt_kd_sr_2_ss) 는 일단 유지함
...
def one_epoch_t_1_3_4(**kargs):
    ...
    # One epoch for t_1_3_4
    # 1. HR image로 SR(T) 학습
    # 2. CN4SRSS 처럼 network guided train + KD
    # 
    ...
    optimizer_l        = kargs['optimizer_l']     # SR + SS
    ...
    for dataloader_items in dataloader_input:
        ...
        if i_mode == "train":
            ...
            with torch.no_grad():
                ...
                if isinstance(tensor_out_t_raw_copy, list):
                    tensor_out_t_copy = tensor_out_t_raw_copy[0]
                else:
                    tensor_out_t_copy = tensor_out_t_raw_copy
            ...
            with torch.cuda.amp.autocast(enabled=True):
                ...
                # Mixup SR (T) and SR (S)
                # 일단 gradient 안끊고 해봄
                tensor_out_sr_mixup = tensor_out_t_copy * HP_MIXUP_A + tensor_out_s * (1 - HP_MIXUP_A)
                tensor_out_seg = model_m(tensor_out_sr_mixup)
            ...
            amp_scaler.scale(loss_s + loss_m).backward(retain_graph=False)
    ...
    if i_mode == "train":
        ...
        # SR (S)
        dict_rb_s[i_mode]["lr"].add_item(optimizer_l.param_groups[0]['lr'])
        ...
        dict_rb_m[i_mode]["lr"].add_item(optimizer_l.param_groups[0]['lr'])
...
def trainer_(**kargs):
    ...
    optimizer_l     = kargs['optimizer_l']   # model_s + model_m (CN4SRSS)
    # criterion_l     = kargs['criterion_l']   # model_s + model_m (CN4SRSS)
    scheduler_l     = kargs['scheduler_l']   # model_s + model_m (CN4SRSS)
    ...
    for i_epoch in range(HP_EPOCH):
        ...
        for i_mode in list_mode:
            print("--- init one_epoch_t_1_3_4", i_mode, "---")
            ...
            is_best =one_epoch_t_1_3_4      (WILL_SAVE_IMAGE = WILL_SAVE_IMAGE
